module ApplicationHelper
  ...
  def floating_action_button
    ...
    content_tag(:div, class: 'fab-add') do
      link_to(
        new_book_path,
        class: fab_class('btn-secondary'), role: :button
  ...
  def decorated_sort_link(search_object, attribute, *args, &block)
    ...
    args[1] ||= {}
    args[1].merge!(
      { class: 'dropdown-item d-flex flex-row-reverse justify-content-end' }
    )
  ...
  def link_to_github
    ...
    options =
      tooltipify(I18n.t('view_on_github')).merge(
  ...
  def hamburger_menu(&block)
    ...
    content_tag(
      :div,
      class: 'ml-3 dropdown', data: { toggle: 'no-collapse' }
    ) do
      concat(
        tag.button(
      ...
      concat(
        content_tag(:ul, class: 'dropdown-menu menu dropdown-menu-right') do
          menu_items
        end
      )
    end
