...
describe Bootstrap::CardDeckBreakpointsComponent, type: :component do
  ...
  describe 'with default breakpoint' do
  ...
  describe 'with breakpoints passed' do
    let(:options) do
      { tag_counter: tag_counter, sm: 3, md: nil, lg: nil, xl: 5 }
    end
    ...
    describe 'with 3 tags' do
      let(:tag_counter) { 3 }
      it { is_expected.to have_css('div', count: 1) }
      it { is_expected.to have_css('.w-100.d-none.d-sm-block.d-xl-none') }
    end
